    ...
    <form action="{{ route('standing_orders.store') }}" method="POST" class="card p-4 shadow-sm">
        ...
        <div class="col-md-4 mb-3">
            ...
            <select name="currency" class="form-select">
                @foreach (\Domain\Enums\Currency::cases() as $currency)
        ...
        <div class="col-md-4 mb-3">
            ...
            <select name="frequency" class="form-select">
                @foreach (\Domain\Enums\PaymentFrequency::cases() as $frequency)
                    <option value="{{ $frequency->value }}"
                        @selected((int) old('frequency', \Domain\Enums\PaymentFrequency::MONTHLY->value) === $frequency->value)>
                        {{ $frequency->label() }}
                    </option>
                @endforeach
            </select>
